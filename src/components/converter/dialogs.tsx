...
export function AuthDialog({ open, onClose, onAuthenticated }: { open: boolean; onClose: () => void; onAuthenticated: () => void }) {
  ...
  return (
    <Dialog
      ...
      description={
        mode === "signup"
          ? "15 pages gratuites chaque mois, sans carte bancaire. Votre relevé reste sur cet appareil."
          : "Votre conversion est conservée dans cet onglet."
      }
    >
      {mode === "signup" ? (
...
export function VerifyDialog({
  ...
  return (
    <Dialog
      ...
      description="Une dernière étape avant votre téléchargement."
    >
      <div className="grid gap-5">
        <div className="flex items-start gap-3 rounded-xl bg-[var(--bg-subtle)] p-4">
          ...
          <p className="text-sm leading-relaxed">
            Nous avons envoyé un lien de confirmation à <strong>{email ?? "votre adresse"}</strong>. Cliquez dessus (il s&apos;ouvre dans un
            nouvel onglet) : votre téléchargement démarrera ici automatiquement.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <Button
            ...
            onClick={async () => {
              ...
              setMessage(
                res.ok
                  ? { tone: "success", text: "E-mail renvoyé. Pensez à vérifier vos courriers indésirables." }
                  : { tone: "error", text: res.data.error ?? "Envoi impossible." },
              );
            }}
          >
            Renvoyer l&apos;e-mail
  ...
  const options: { id: ProductId; title: string; price: string; note: string; highlight?: boolean; features: string[] }[] = [
    {
      ...
      features: ["Valable 12 mois", "Tous les formats", "Plusieurs relevés et fusion"],
    },
    {
      id: "pro_monthly",
    ...
    <Dialog open={open} onClose={onClose} title="Passez à la vitesse supérieure" description={reason} className="max-w-3xl">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {options.map((o) => (
          <div
            key={o.id}
            className={o.highlight ? "rounded-2xl border-2 border-brand-500 p-5" : "rounded-2xl border border-[var(--border)] p-5"}
          >
            ...
            <Button
              ...
              variant={o.highlight ? "primary" : "secondary"}
              loading={loading === o.id}
              disabled={!!loading && loading !== o.id}
              onClick={() => go(o.id)}
            >
              Choisir
            </Button>
...
export function ReportLayoutDialog({ open, onClose, statement }: { open: boolean; onClose: () => void; statement?: ParsedStatement }) {
  ...
  return (
    <Dialog
      open={open}
      onClose={onClose}
      title="Signaler un relevé mal lu"
      description="Aidez-nous à améliorer la lecture de ce type de relevé."
    >
      {state === "sent" ? (
        <Alert tone="success" title="Merci !">
          Votre signalement anonyme a bien été envoyé.
        </Alert>
      ) : (
        <form
        ...
        >
          <p className="text-sm leading-relaxed text-muted">
            Seule la <strong>structure</strong> du résultat est envoyée : nombre de pages et d&apos;opérations, colonnes détectées, statut
            du contrôle et avertissements. Aucun libellé, montant, nom ou numéro de compte.
          </p>
          <pre className="max-h-40 overflow-auto rounded-xl bg-[var(--bg-subtle)] p-3 text-xs">{JSON.stringify(summary, null, 2)}</pre>
          <div>
            <label htmlFor="layout-comment" className="mb-1.5 block text-sm font-semibold">
              Commentaire (facultatif, sans donnée personnelle)
            </label>
            <Textarea
              id="layout-comment"
              value={comment}
              onChange={(e) => setComment(e.target.value.slice(0, 1000))}
              placeholder="Ex. : les montants de la colonne crédit sont lus comme des débits."
            />
          </div>
