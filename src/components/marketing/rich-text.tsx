...
export function RichText({ text }: { text: string }) {
  ...
  while ((m = re.exec(text))) {
    ...
    if (m[1]) out.push(<strong key={i++}>{m[1]}</strong>);
      ...
      const href = m[3];
      if (href.startsWith("/")) out.push(<Link key={i++} href={href}>{m[2]}</Link>);
      else if (/^https:\/\//.test(href)) out.push(<a key={i++} href={href} rel="noopener noreferrer nofollow" target="_blank">{m[2]}</a>);
            ...
            return (
              <div key={i} className="my-6 overflow-x-auto rounded-xl border border-[var(--border)]">
                <table>
                  ...
                  <tbody>{b.rows.map((r, j) => <tr key={j}>{r.map((c, k) => <td key={k}><RichText text={c} /></td>)}</tr>)}</tbody>
                </table>
              </div>
            );
          case "cta":
            return (
              <div key={i} className="my-8 rounded-2xl border border-brand-200 bg-brand-50 p-6 dark:border-brand-800/60 dark:bg-brand-950/40">
                <p className="!mb-1 text-lg font-bold">{b.title}</p>
                <p className="!mb-4 text-muted">{b.text}</p>
                <ButtonLink href={b.href} data-cta="content-cta">{b.label}</ButtonLink>
              </div>
            );
        }
      })}
