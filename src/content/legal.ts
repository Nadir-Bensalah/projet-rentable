import { legalIdentity } from "@/config/site";
...
/** Slugs of the legal pages (static list for the sitemap and footer). */
export const LEGAL_SLUGS = ["mentions-legales", "confidentialite", "cookies", "cgu", "cgv", "remboursement"] as const;

/**
 * Builds the legal pages from the publisher identity read at request time
 * (see legalIdentity in src/config/site.ts), so that a single Docker image can be
 * configured with runtime environment variables.
 */
export function legalPages(): LegalPage[] {
  const id = legalIdentity();
  const CONTACT_EMAIL = id.contactEmail;
  const PUBLISHER = id.publisherName;
  const STATUS = id.status;
  const SIREN = id.siren;
  const ADDRESS = id.address;
  const VAT = id.vat;
  const DIRECTOR = id.director;
  const APP_HOST = id.host;
  const DB_HOST = id.dbHost;
  const PAYMENT_PROVIDER = id.paymentProvider;
  const EMAIL_PROVIDER = id.emailProvider;
  ...
  return [
    // ---------------------------------------------------------------------------
    {
      ...
      blocks: [
        ...
        { type: "h2", id: "hebergement", text: "Hébergement" },
        {
          ...
          rows: [
            ["Hébergement de l'application", APP_HOST],
            ["Hébergement de la base de données", DB_HOST],
          ],
        },
        {
          type: "p",
      ...
      blocks: [
        ...
        { type: "h2", id: "donnees", text: "Données que nous traitons" },
        {
          ...
          rows: [
            ...
            [
              "Mesure d'audience",
              "Nom de l'événement, identifiant aléatoire de visite (propre à l'onglet), page visitée, site de provenance, paramètres utm, petites propriétés (ex. format choisi, nombre de pages) — jamais rattachés au compte, jamais le contenu d'un relevé",
            ...
            [
              ...
              "Opérations du compte (inscription, connexion, export décompté, achat, résiliation, échec de paiement) avec l'identifiant de compte et de petites propriétés (offre, montant, format)",
              "Gérer le service (chiffre d'affaires, rétention, support) et prévenir les abus",
              "Intérêt légitime ; exécution du contrat",
            ],
            [
              "Consentement à l'exécution immédiate",
              "Offre choisie, date et version du texte accepté avant le paiement",
              "Prouver la renonciation au droit de rétractation (art. L221-28 du Code de la consommation)",
              "Obligation légale ; intérêt légitime (preuve)",
            ],
          ],
        ...
        { type: "h2", id: "conservation", text: "Durées de conservation" },
        {
          ...
          head: ["Données", "Durée"],
          rows: [
            ["Compte, crédits, événements d'export, abonnements", "Jusqu'à la suppression du compte"],
            ["Journal des e-mails envoyés (adresse, type, date)", "13 mois, puis suppression automatique"],
            ["Notifications techniques du prestataire de paiement", "13 mois après traitement"],
            ["Sessions", "30 jours"],
            ...
            ["Messages de contact", "3 ans, puis suppression automatique"],
            [
              "Rapports de mise en page anonymisés",
              "3 ans, puis suppression automatique (le lien avec le compte est supprimé à la suppression du compte)",
            ],
          ],
        },
    ...
    // ---------------------------------------------------------------------------
    {
      ...
      blocks: [
        ...
        { type: "h2", id: "stockage", text: "Stockage dans le navigateur" },
        {
          ...
          rows: [
            ...
            [
              "rv_ref",
              "sessionStorage",
              "Code de parrainage présent dans le lien d'invitation, repris à l'inscription",
              "Jusqu'à la fermeture de l'onglet",
            ],
        ...
        {
          type: "p",
          text: "Nous mesurons la fréquentation avec un outil **interne** (first-party) : les événements envoyés par votre navigateur (pages vues, étapes clés comme l'ouverture d'un relevé) partent vers nos propres serveurs avec un identifiant aléatoire propre à l'onglet. Ils ne sont **jamais rattachés à votre compte**, même si vous êtes connecté, ne contiennent jamais le contenu de vos relevés, ne sont transmis à aucun tiers et sont supprimés automatiquement au bout de 13 mois.",
        },
        {
          type: "p",
          text: "Distinctement de la mesure d'audience, le serveur enregistre les opérations de votre compte nécessaires au service et à sa gestion (inscription, connexion, exports décomptés, achats, résiliations), pour exécuter le contrat, établir les statistiques de gestion (chiffre d'affaires, rétention) et prévenir les abus. Ces enregistrements ne dépendent pas de votre navigateur et ne sont donc pas concernés par le bouton d'opposition ci-dessous ; ils sont décrits dans la [politique de confidentialité](/confidentialite).",
        },
        {
          type: "p",
    ...
    // ---------------------------------------------------------------------------
    {
      ...
      blocks: [
        ...
        },
        {
          type: "ul",
          items: [
            "Une **page** correspond à une page d'un fichier PDF exporté. Le ré-export du même fichier au cours du même mois n'est pas décompté à nouveau, dans la limite du nombre de pages décomptées la première fois et de 10 ré-exports par fichier et par mois ; au-delà, les pages sont décomptées normalement.",
        ...
        { type: "h2", id: "abonnements", text: "5. Abonnements : renouvellement et résiliation" },
        {
          ...
          items: [
            ...
            "En cas d'échec de paiement, le prestataire relance le paiement ; l'accès est maintenu pendant 14 jours après l'échéance, puis suspendu jusqu'à régularisation.",
          ],
        },
        {
          type: "p",
          text: "Pour les abonnements annuels, le client est informé par e-mail, au plus tôt trois mois et au plus tard un mois avant l'échéance, de la possibilité de ne pas reconduire le contrat (art. L215-1 du Code de la consommation). La résiliation se fait en ligne, depuis la rubrique Abonnement du compte (art. L215-1-1). [À CONFIRMER par un professionnel du droit]",
        },
        ...
        { type: "h2", id: "retractation", text: "6. Droit de rétractation (consommateurs)" },
        {
          ...
          text: "La rédaction de cet article et le mécanisme de recueil du consentement au moment du paiement (case à cocher obligatoire, consentement horodaté et confirmé par e-mail) doivent être validés par un professionnel du droit.",
        },
        {
          type: "p",
        ...
        {
          type: "p",
          text: "Toutefois, Relevéo fournit un contenu et un service numériques **immédiatement accessibles** après le paiement. Conformément à l'article L221-28, 13° du Code de la consommation, lors de la commande, le consommateur **demande expressément l'exécution immédiate** du contrat et **reconnaît qu'il perd son droit de rétractation** dès qu'il commence à utiliser les pages achetées (premier export décompté sur l'offre payante), en cochant une case prévue à cet effet avant d'accéder au paiement. Ce consentement est horodaté et confirmé dans l'e-mail de confirmation de la commande. Tant qu'aucune page payante n'a été utilisée, le droit de rétractation peut être exercé dans le délai de 14 jours. [À CONFIRMER : fondement exact (L221-28 1° et/ou 13°) et régime pour les abonnements]",
        },
        {
          type: "p",
